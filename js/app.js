...
(() => {
  ...
  function reflowFoundation(root) {
    if (window.Foundation?.reInit) Foundation.reInit(root);
  }
  ...
  // ---- The unified init() / unload() you asked about ----------------------
  function init(root = document) {
    // Order: DOM upgrades → widgets → frameworks → analytics
    initSwipers(root);
    initMenus(root);
    reflowFoundation(root);
    trackPageview();
  }
  ...
  // Optional: always scroll to top on nav (unless you have a smarter scroll plugin)
  swup.hooks.on('page:view', () => window.scrollTo(0, 0));
